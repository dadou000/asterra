...
func _place(direction: Vector3) -> void:
	...
	if kind == Kind.BOAT:
		# Test hull is centred slightly above the zero-metre ocean surface.
		var r := Planet.cfg.planet_radius + 0.45
		global_position = Frames.to_render(Vec3D.new(d.x * r, d.y * r, d.z * r))
	else:
		var clearance := 90.0 if kind == Kind.PLANE else 1.0
...
func _apply_boat(_dt: float, up: Vector3, altitude: float) -> void:
	_apply_radial_alignment(up)
	# Simple test buoyancy intentionally avoids the old OceanBuoyancy3D bathymetry
	# path, which still instantiates CPU TerrainDetail. Zero metres is the ocean
	# datum; this is sufficient for wind/water/floating-origin vehicle tests.
	var waterline := 0.45
...
func _sample_wind(direction: Vector3, altitude: float) -> Vector3:
	# Use the native weather sampler when the weather branch is present, but keep
	# this harness portable on the current terrain branch by falling back to the
	# baked climate wind field when /root/WeatherSystem is absent.
