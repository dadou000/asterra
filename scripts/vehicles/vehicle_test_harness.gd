...
## this keeps terrain, sky, atmosphere, floating origin and HUD centred on the
## vehicle without introducing a second observer API.
...
func _find_nearby_ocean(center: Vector3) -> Vector3:
	# This is a one-shot spawn search and deliberately uses the resident coarse
	# height rather than flooding the single-point asynchronous GPU query.
	if TerrainContactSampler.coarse_height(center) < -2.0:
		return center
	...
	var best_height := TerrainContactSampler.coarse_height(center)
	var radii: Array[float] = [2000.0, 5000.0, 10000.0, 20000.0, 40000.0, 80000.0]
	for radius_m: float in radii:
		for i: int in 24:
			var a := TAU * float(i) / 24.0
			var offset := east * cos(a) + north * sin(a)
			var angle := radius_m / float(Planet.cfg.planet_radius)
			var d := (center * cos(angle) + offset * sin(angle)).normalized()
			var h := TerrainContactSampler.coarse_height(d)
			if h < best_height:
				best_height = h
				best = d
			if h < -2.0:
...
func _kind_name(kind: WindVehicleTest.Kind) -> String:
	match kind:
		WindVehicleTest.Kind.BOAT:
			return "boat"
		WindVehicleTest.Kind.CAR:
			return "car"
		_:
			return "plane"
