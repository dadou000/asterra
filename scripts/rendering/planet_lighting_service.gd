...
## The LUT is rebuilt only when the resident body's geometry changes.
...
func _ready() -> void:
	...
	RenderingServer.global_shader_parameter_add(
		LUT_NAME, RenderingServer.GLOBAL_VAR_TYPE_SAMPLER2D, _lut)
	RenderingServer.global_shader_parameter_add(
		SUN_DIR_NAME, RenderingServer.GLOBAL_VAR_TYPE_VEC3, Vector3(1.0, 0.0, 0.0))
...
func _sync_lut_if_needed() -> void:
	...
	if is_equal_approx(radius, _last_radius) and is_equal_approx(atmosphere_height, _last_atmosphere_height):
		return
	_last_radius = radius
...
func _build_transmittance_lut(planet_radius: float, atmosphere_height: float) -> ImageTexture:
	...
	for y in range(LUT_HEIGHT):
		...
		for x in range(LUT_WIDTH):
			...
			image.set_pixel(x, y, Color(exp(-tau.x), exp(-tau.y), exp(-tau.z), 1.0))
