...
##
## WeatherSystem publishes a global CPU weather array as part of its normal UI/
## analysis path. This system samples that already-resident array at PlanetGrid
## cell centres, converts precipitation B into m/s water-equivalent input, and
## advances PlanetHydrologyStore in simulation time. No GPU readback is involved.
##
...
func _rebuild_store() -> void:
	...
	_store.set_climatology_fallback_enabled(not WeatherSystem.native_available)
...
func _maybe_refresh_weather_snapshot(simulation_seconds: float) -> void:
	...
	var native_now := WeatherSystem.native_available
	_store.set_climatology_fallback_enabled(not native_now)
...
func _refresh_weather_snapshot(simulation_seconds: float) -> void:
	...
	if not WeatherSystem.native_available:
		...
		_last_weather_revision = WeatherSystem.global_state_revision
	...
	var scale_mps := maxf(maximum_precipitation_mm_h, 0.0) \
		* MM_H_TO_M_S * maxf(weather_gain, 0.0)
	for c in n:
