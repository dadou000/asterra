...
## untouched. While any transaction is pending, coarse stepping is paused rather
## than allowing reserved water to route away under an asynchronous GPU handoff.
## The planetary coarse cadence is slow enough for this conservative gate; a later
## per-cell lock can relax the global pause if profiling shows it is necessary.
##
## Pending reservations are intentionally NOT serialized. snapshot() fails closed
## while ownership is unresolved, so save/load can never resurrect coarse water
## that may already have been staged into a fine representation.
...
func prepare_promotion(cell: int, requested_volume_m3: float) -> Dictionary:
	if not initialized:
		return _promotion_error(ERR_UNCONFIGURED, "store_unconfigured")
	if cell < 0 or cell >= cell_count() or fields.elev[cell] <= 0.0:
		return _promotion_error(ERR_INVALID_PARAMETER, "invalid_land_cell")
	if not is_finite(requested_volume_m3) or requested_volume_m3 <= 0.0:
		return _promotion_error(ERR_INVALID_PARAMETER, "invalid_volume")
	...
	if requested_volume_m3 > available + epsilon:
		var rejected := _promotion_error(ERR_CANT_ACQUIRE_RESOURCE, "insufficient_free_water")
...
func commit_promotion(transaction_id: int) -> Dictionary:
	if not _pending_promotions.has(transaction_id):
		return _promotion_error(ERR_DOES_NOT_EXIST, "unknown_transaction")
	...
	if surface_storage_m3[cell] + epsilon < surface_volume \
		...
		return _promotion_error(ERR_INVALID_DATA, "reserved_storage_invariant_broken")
	...
	_release_reservation(transaction)
...
## physical water is modified because prepare_promotion() never debits storage.
...
## Symmetric representation transfer used when a fine domain is collapsed back
## into the planetary store. The caller must first conservatively reduce/remove
## this exact volume from fine SWE; this method does not inspect GPU state.
func accept_demotion(cell: int, surface_volume_m3: float,
		channel_volume_m3: float = 0.0) -> Dictionary:
	if not initialized:
		return _promotion_error(ERR_UNCONFIGURED, "store_unconfigured")
	if not _pending_promotions.is_empty():
		return _promotion_error(ERR_BUSY, "promotion_transaction_pending")
	if cell < 0 or cell >= cell_count() or fields.elev[cell] <= 0.0:
		return _promotion_error(ERR_INVALID_PARAMETER, "invalid_land_cell")
	...
	surface_storage_m3[cell] += surface_volume_m3
	channel_storage_m3[cell] += channel_volume_m3
	cumulative_demoted_from_fine_m3 += total
	return {
		"error": OK,
		"cell": cell,
		"accepted_volume_m3": total,
		"surface_volume_m3": surface_volume_m3,
		"channel_volume_m3": channel_volume_m3,
		"coarse_storage_m3": total_storage_m3(),
...
func snapshot() -> Dictionary:
	# Saving unresolved ownership is unsafe: the fine side may have accepted a seed
	# while the coarse debit has not yet been acknowledged. Fail closed.
	if not _pending_promotions.is_empty():
		return {}
...
func stats() -> Dictionary:
	...
	out["ownership_transactions"] = {
		"pending": _pending_promotions.size(),
		...
		"snapshot_blocked": not _pending_promotions.is_empty(),
	}
...
func _release_reservation(transaction: Dictionary) -> void:
...
func _promotion_error(error: Error, reason: String) -> Dictionary:
	return {
		"error": error,
		"reason": reason,
	}
