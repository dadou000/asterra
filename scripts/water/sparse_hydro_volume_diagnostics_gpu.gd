class_name SparseHydroVolumeDiagnosticsGPU
extends Node
## Representation-wide Phase 3 water-volume diagnostic.
##
## Reads the sparse atlas conservative state plus occupancy on the global
## RenderingDevice and returns only one four-byte FP32 volume asynchronously.
## Unoccupied/recycled slots are excluded even if their state buffers retain stale
## bytes. The solver, occupancy and state resources remain externally owned.
...
func initialize(state_rid: RID, occupancy_rid: RID, capacity: int,
	...
	var reduce_file: RDShaderFile = load("res://shaders/water/sparse_hydro_mass_reduce.glsl")
	var final_file: RDShaderFile = load("res://shaders/water/sparse_hydro_mass_finalize.glsl")
	...
	_total_cells = capacity * _cells_per_tile
	_cell_size_m = cell_size_m
	_partial_count = int(ceil(float(_total_cells) / float(LOCAL_SIZE)))
	_init_pending = true
	RenderingServer.call_on_render_thread(Callable(self, &"_init_render_thread").bind(
		reduce_spirv, final_spirv, _make_params()))
	return OK


func initialize_from_atlas(atlas: SparseHydroAtlasGPU, use_state_b: bool = false) -> Error:
...
func gpu_bytes_estimate() -> int:
	return _partial_count * 4 + 4 + PARAM_BYTES
...
func _make_params() -> PackedByteArray:
	...
	bytes.encode_u32(0, _total_cells)
	bytes.encode_u32(4, _cells_per_tile)
	bytes.encode_u32(8, _capacity)
