...
## dependent ROM coupling. Constraint axes are defined in anatomical joint-local
## frames and coupling is measured with swing-twist decomposition, not raw Euler
## angles. The mouse manipulator applies a damped spring at the grabbed point.
...
func _update_overlay() -> void:
	if _overlay_label == null:
		return
	var limits_text := "ON" if _anatomical_limits_enabled else "OFF"
	var twist_text := "ON" if _axial_twist_enabled else "LOCKED"
	var grabbed_text := _grabbed_role if _grabbed_body != null else "none"
	_overlay_label.text = (
...
func _build_ragdoll() -> void:
	...
	# Self-collision remains disabled for this joint-isolation stage. This makes
	# limit/frame errors visible without body-body contact noise.
...
func _joint_specs() -> Array:
	# Every joint defines an anatomical frame:
	#   local X = primary flexion/extension axis
	#   local Y = long-axis twist
	#   local Z = lateral swing / abduction-adduction / varus-valgus
	# Most mockup segments are vertical, so X=world right and Y=world up. The
	# clavicles are horizontal and therefore receive a genuinely rotated frame.
	return [
		...
		# Horizontal clavicles: Y follows the clavicle shaft, X is anterior-posterior,
		# and Z becomes the elevation/protraction companion axis.
		_joint("chest_left_clavicle", "chest", "left_clavicle", Vector3(-0.145,1.58,0), Vector3(-12,-30,-25), Vector3(25,35,25), Vector3.FORWARD, Vector3.LEFT),
		_joint("chest_right_clavicle", "chest", "right_clavicle", Vector3(0.145,1.58,0), Vector3(-12,-30,-25), Vector3(25,35,25), Vector3.FORWARD, Vector3.RIGHT),
		...
		# The single forearm body carries pronation/supination around local Y. This
		# approximates the proximal+distal radioulnar joints without adding bodies.
...
func _make_joint(spec: Dictionary) -> void:
	...
	var frame_basis := _make_anatomical_basis(
		Vector3(spec["flex_axis_world"]),
		Vector3(spec["twist_axis_world"])
	)
	joint.transform = Transform3D(frame_basis, Vector3(spec["anchor"]) + SPAWN_OFFSET)
	add_child(joint)
	...
	# Store the neutral anatomical frame relative to each body. Measuring current
	# motion through these frames removes parent/body roll from the ROM calculation.
	var joint_world_basis := joint.global_transform.basis.orthonormalized()
...
func _make_anatomical_basis(flex_axis_world: Vector3, twist_axis_world: Vector3) -> Basis:
	var y_axis := twist_axis_world.normalized()
	var x_axis := flex_axis_world - y_axis * flex_axis_world.dot(y_axis)
...
func _validate_joint_spec(spec: Dictionary) -> void:
	...
	assert(Vector3(spec["flex_axis_world"]).length_squared() > 0.5, "Joint flex axis is invalid: %s" % String(spec["name"]))
	assert(Vector3(spec["twist_axis_world"]).length_squared() > 0.5, "Joint twist axis is invalid: %s" % String(spec["name"]))
...
func _set_joint_angular_limits_enabled(joint: Generic6DOFJoint3D, enabled: bool) -> void:
	for axis in ["x", "y", "z"]:
...
func _relative_joint_quaternion(joint_name: String) -> Quaternion:
	...
	var frame_a_world := (parent_body.global_transform.basis * frame_a_local).orthonormalized()
	var frame_b_world := (child_body.global_transform.basis * frame_b_local).orthonormalized()
	var relative := (frame_a_world.inverse() * frame_b_world).orthonormalized()
	return relative.get_rotation_quaternion().normalized()


func _joint_swing_twist(joint_name: String) -> Dictionary:
	# Local Y is defined as the anatomical long/twist axis for every joint.
	var q := _relative_joint_quaternion(joint_name)
	var twist_axis := Vector3.UP
	var q_vector := Vector3(q.x, q.y, q.z)
	var projected := twist_axis * q_vector.dot(twist_axis)
	var norm_sq := projected.length_squared() + q.w * q.w
	var twist := Quaternion(0.0, 0.0, 0.0, 1.0)
	...
	var swing := (q * twist.inverse()).normalized()
	var swing_direction: Vector3 = swing * Vector3.UP
	var flex_deg := rad_to_deg(atan2(swing_direction.z, swing_direction.y))
	var lateral_denominator := sqrt(maxf(0.0000001, swing_direction.y * swing_direction.y + swing_direction.z * swing_direction.z))
	var lateral_deg := rad_to_deg(atan2(-swing_direction.x, lateral_denominator))
	var twist_vector := Vector3(twist.x, twist.y, twist.z)
	var twist_deg := rad_to_deg(2.0 * atan2(twist_vector.dot(twist_axis), twist.w))
	twist_deg = rad_to_deg(wrapf(deg_to_rad(twist_deg), -PI, PI))
	return {
		"flex_deg": flex_deg,
...
func _update_knee_coupling(joint_name: String) -> void:
	...
	var motion := _joint_swing_twist(joint_name)
	var flexion_deg := clampf(-float(motion["flex_deg"]), 0.0, 145.0)
	# Near extension the screw-home mechanism and collateral ligaments permit very
	# little axial or frontal motion. Flexion progressively unlocks a small amount.
	var unlock := smoothstep(8.0, 95.0, flexion_deg)
	var axial_deg := lerpf(1.5, 10.0, unlock)
	var frontal_deg := lerpf(1.5, 4.0, unlock)
...
func _update_hip_coupling(joint_name: String, side: float) -> void:
	...
	var joint: Generic6DOFJoint3D = _joints[joint_name]
	var motion := _joint_swing_twist(joint_name)
	var flexion_deg := clampf(float(motion["flex_deg"]), 0.0, 125.0)
	var deep := smoothstep(65.0, 120.0, flexion_deg)
	var internal_deg := lerpf(40.0, 20.0, deep)
	var external_deg := lerpf(50.0, 26.0, deep)
	var abduction_deg := lerpf(45.0, 25.0, deep)
	var adduction_deg := lerpf(30.0, 16.0, deep)
	if side < 0.0:
...
func _update_shoulder_coupling(joint_name: String) -> void:
	...
	var joint: Generic6DOFJoint3D = _joints[joint_name]
	var motion := _joint_swing_twist(joint_name)
	var flexion_fraction := clampf(absf(float(motion["flex_deg"])) / 135.0, 0.0, 1.0)
	var abduction_fraction := clampf(absf(float(motion["lateral_deg"])) / 90.0, 0.0, 1.0)
	var elevation := maxf(flexion_fraction, abduction_fraction)
	# Prevent the rectangular 6DOF envelope from combining near-maximal elevation
	# with near-maximal axial roll. This was the major visible twisting artifact.
	var tighten := smoothstep(0.48, 1.0, elevation)
	var axial_deg := lerpf(80.0, 32.0, tighten)
	_set_angular_range(joint, "y", -axial_deg, axial_deg)
...
func _update_ankle_coupling(joint_name: String) -> void:
	...
	var spec: Dictionary = _joint_specs_by_name[joint_name]
	var motion := _joint_swing_twist(joint_name)
	var sagittal_deg := float(motion["flex_deg"])
	var center_distance := absf((sagittal_deg - -12.5) / 37.5)
	var tighten := smoothstep(0.48, 1.0, clampf(center_distance, 0.0, 1.0))
	var lower: Vector3 = spec["lower"]
	var upper: Vector3 = spec["upper"]
	var y_scale := lerpf(1.0, 0.45, tighten)
	var z_scale := lerpf(1.0, 0.58, tighten)
...
func _set_anatomical_limits_enabled(enabled: bool) -> void:
	...
	if enabled:
		...
		for joint_name in _joints:
			_set_joint_angular_limits_enabled(_joints[joint_name], false)
	_update_overlay()
...
func _begin_grab(screen_position: Vector2) -> void:
	...
	var query := PhysicsRayQueryParameters3D.create(ray_from, ray_from + ray_direction * RAY_LENGTH)
	...
	_grabbed_body = collider as RigidBody3D
	_grabbed_role = String(_grabbed_body.name)
	var hit_position: Vector3 = hit["position"]
	_grab_local_point = _grabbed_body.to_local(hit_position)
	_grab_distance = clampf(ray_from.distance_to(hit_position), MIN_GRAB_DISTANCE, MAX_GRAB_DISTANCE)
...
func _update_physics_grab() -> void:
	...
	var world_point := _grabbed_body.to_global(_grab_local_point)
	var offset := world_point - _grabbed_body.global_position
	var point_velocity := _grabbed_body.linear_velocity + _grabbed_body.angular_velocity.cross(offset)
	var error := _grab_target_world - world_point
	var stiffness := GRAB_STIFFNESS_PER_KG * _grabbed_body.mass
	var damping := GRAB_DAMPING_PER_KG * _grabbed_body.mass
	var force := error * stiffness - point_velocity * damping
	var max_force := GRAB_MAX_FORCE_PER_KG * _grabbed_body.mass
...
func _assert_articulation_contract() -> void:
	...
	assert(_joint_frame_a_local.size() == EXPECTED_JOINT_COUNT, "Every joint needs parent anatomical frame")
	assert(_joint_frame_b_local.size() == EXPECTED_JOINT_COUNT, "Every joint needs child anatomical frame")
	var total_mass := 0.0
	...
	print("T toggles axial twist independently for diagnosis")
...
func _reset_ragdoll() -> void:
	...
	# Small asymmetric kick exposes frame/constraint problems without dominating
	# the passive articulation test.
