...
##
## Runtime terrain geometry should mostly be an I/O problem, not procedural
## synthesis. This store canonicalises requested heights onto the cube-sphere
## lattice, groups them into 32x32-cell tiles, and keeps pristine results on disk.
## Visual requests never synchronously touch disk or synthesize terrain: they use
## the nearest resident parent, queue missing pages, and sharpen later.
...
# request key -> current best priority score. In-flight requests remain present so
