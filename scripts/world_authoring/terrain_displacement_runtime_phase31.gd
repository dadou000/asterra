...
## inputs already used by Phase 30. Phase 32 adds an unconnected production
## settings node; it is metadata for the production shader and must not make an
## otherwise canonical shape graph enter the displacement bytecode interpreter.
...
func _is_identity_shape_slot(slot: Resource) -> bool:
	...
	for node_value: Variant in nodes:
		...
		match node_type:
			...
			"PRODUCTION_GEOMORPH_SETTINGS": settings_count += 1
			"GAME_INPUT":
