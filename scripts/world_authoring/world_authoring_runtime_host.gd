extends "res://scripts/world_authoring/world_authoring_runtime_host_base.gd"
## Thin pre-0.1.0 runtime entry point. The production runtime host implementation is
## preserved byte-for-byte in world_authoring_runtime_host_base.gd; only editor
## construction is overridden so Main and PlanetStudio.tscn open the same composed
## Planet Studio workbench.

const PRE01_EDITOR_SCRIPT := preload("res://scripts/world_authoring/world_authoring_editor_live_pre01.gd")
...
func _open_live_editor(player: Node) -> void:
	...
	_layer.layer = 90
	add_child(_layer)
	var live_editor: Control = PRE01_EDITOR_SCRIPT.new()
