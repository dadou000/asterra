...
func _phase29_build_graph_editor(slot: Resource) -> void:
	...
	if _phase28_domain == SHADER_SLOT_MODEL.Domain.DISPLACEMENT:
		hint.text = "SIMPLE and DETAILED edit the same exact terrain as NODE GRAPH. Base Terrain native stages keep the safe Phase 40 Add / Scale / constant Blend contract. Additional authored displacement flows can use LATITUDE BAND, LONGITUDE BAND and GEOGRAPHIC REGION masks in normalized planet space. Geographic Region combines both axes in one node and remains seam-safe across ±180°. Rendering and contact/physics evaluate the same lowered bytecode. PREVIEW never replaces the applied world unless the candidate is valid."
	else:
