extends Node
## Phase 41 UI regression for Ring Area in the real Planet Studio path.
...
const RING_PICKER_TYPE := "RING_MASK"
...
func _run() -> void:
	OS.set_environment("ASTERRA_AUTHORING_RECOVERY_PATH",
		"user://world_authoring/tests/phase41_ring_ui_recovery_%d.tres" % OS.get_process_id())
	...
	var slot: Resource = terrain.call("create_shader_slot",
		SLOT.Domain.DISPLACEMENT, "CI Ring Area UI") as Resource
	...
	var ring_index: int = _picker_index(picker, RING_PICKER_TYPE)
	if ring_index < 0 or picker.get_item_text(ring_index) != "Mask  ·  Ring Area":
		_fail("generic displacement picker is missing Ring Area")
		return
	picker.select(ring_index)
	graph_editor.call("_on_add_node")
	await _frames(5)

	var graph: Resource = slot.get(&"graph") as Resource
	var ring_id: String = _ring_node_id(graph)
	if ring_id.is_empty():
		_fail("Ring Area did not serialize as LATITUDE_MASK axis=ring")
		return
	var parameters: Dictionary = _node_parameters(graph, ring_id)
	if not is_equal_approx(float(parameters.get("center_latitude_deg", 999.0)), 0.0) \
			...
			or not is_equal_approx(float(parameters.get("inner_radius_deg", 999.0)), 10.0) \
			or not is_equal_approx(float(parameters.get("outer_radius_deg", 999.0)), 20.0) \
			or not is_equal_approx(float(parameters.get("feather_deg", 999.0)), 5.0) \
			or bool(parameters.get("invert", true)):
		_fail("Ring Area defaults changed")
		return

	graph_editor = _find_script_node(editor, GRAPH_EDITOR_PATH)
	var ring_node: GraphNode = _find_graph_node(graph_editor, "RING AREA MASK")
	if ring_node == null:
		_fail("serialized ring variant was not rendered as RING AREA MASK")
		return
	var center_latitude: SpinBox = _spin_for_label(ring_node, "Center latitude")
	var center_longitude: SpinBox = _spin_for_label(ring_node, "Center longitude")
	var inner_radius: SpinBox = _spin_for_label(ring_node, "Inner radius")
	var outer_radius: SpinBox = _spin_for_label(ring_node, "Outer radius")
	var feather: SpinBox = _spin_for_label(ring_node, "Feather")
	if center_latitude == null or center_longitude == null or inner_radius == null \
			or outer_radius == null or feather == null:
		_fail("Ring Area is missing one or more degree controls")
		return
	for spin: SpinBox in [center_latitude, center_longitude, inner_radius, outer_radius, feather]:
		if spin.suffix != "°":
			_fail("Ring Area contains a control without degree units")
			return
	if not is_equal_approx(center_latitude.min_value, -90.0) \
			...
			or not is_equal_approx(inner_radius.min_value, 0.0) \
			or not is_equal_approx(inner_radius.max_value, 180.0) \
			or not is_equal_approx(outer_radius.min_value, 0.0) \
			or not is_equal_approx(outer_radius.max_value, 180.0) \
			or not is_equal_approx(feather.min_value, 0.0) \
			or not is_equal_approx(feather.max_value, 180.0):
		_fail("Ring Area degree ranges do not match spherical runtime limits")
		return
	...
	inner_radius.emit_signal("value_changed", 12.5)
	outer_radius.emit_signal("value_changed", 32.5)
	feather.emit_signal("value_changed", 7.5)
	var invert: CheckButton = _find_invert(ring_node)
	if invert == null:
		_fail("Ring Area invert control is missing")
		return
	invert.emit_signal("toggled", true)
	await _frames(2)
	parameters = _node_parameters(graph, ring_id)
	if not is_equal_approx(float(parameters.get("center_latitude_deg", 0.0)), 65.0) \
			...
			or not is_equal_approx(float(parameters.get("inner_radius_deg", 0.0)), 12.5) \
			or not is_equal_approx(float(parameters.get("outer_radius_deg", 0.0)), 32.5) \
			or not is_equal_approx(float(parameters.get("feather_deg", 0.0)), 7.5) \
			or not bool(parameters.get("invert", false)):
		_fail("Ring Area controls did not update the staged graph")
		return
	...
	if _picker_index(base_picker, RING_PICKER_TYPE) >= 0:
		_fail("resident Base Terrain incorrectly exposes Ring Area")
		return

	print("RING_MASK_PHASE41_UI_OK: picker, canonical serialization, staged center/inner/outer/feather controls, inversion and Base Terrain safety boundary match")
	editor.queue_free()
	await _frames(2)
	get_tree().quit(0)
...
func _ring_node_id(graph: Resource) -> String:
	if graph == null:
		return ""
	for value: Variant in graph.get(&"nodes") as Array:
		...
		if String(node.get("type", "")) == "LATITUDE_MASK" \
				and String(parameters.get("axis", "latitude")) == "ring":
...
func _fail(message: String) -> void:
	push_error(message)
	print("RING_MASK_PHASE41_UI_FAILED: %s" % message)
	get_tree().quit(1)
