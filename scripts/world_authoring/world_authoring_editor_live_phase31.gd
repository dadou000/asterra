...
## Phase 31/32: renderer-owned terrain stages are explicit graph sources and
...
func _phase29_build_graph_editor(slot: Resource) -> void:
	...
	if _phase28_domain == SHADER_SLOT_MODEL.Domain.DISPLACEMENT:
		hint.text = "GENERATED TERRAIN exposes the existing macro + geomorph surface, with the real geomorph band settings beside it. SCULPT / EDIT DELTA remains explicit. Defaults reproduce the current renderer; edits change the authoritative render/contact terrain in place."
	else:
		hint.text = "The existing production surface is fully exposed: PBR outputs, classifier, microrelief, anti-tiling, rock PBR, scanned PBR, exact scan textures, raw world fields and graph math/texture nodes. Reset Flow restores the current production defaults."
