extends Node
## Standalone deterministic Phase 42A regression.
##
## Keep this file inheritance-free: a validation script must be able to report a
## parse/load failure immediately instead of depending on another validation script
## that may itself be the thing Godot cannot resolve.
...
func _validate_cache_invalidation() -> String:
	...
	# Do not enter the SceneTree. _ready() initializes the CPU cache queues; under
	# the isolated GL-compatibility project it cannot schedule RenderingDevice work.
	cache.call("_ready")
	var fixture: Dictionary = _fixture()
