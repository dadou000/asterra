...
## until spatial native-stage lowering exists.
...
func _run() -> void:
	...
	if editor_script == null or editor_script.resource_path != EXPECTED_EDITOR_PATH:
		_fail("PlanetStudio.tscn is not using the Phase 41 live editor wrapper.")
		return
	...
	if terrain == null:
		_fail("Phase 41 Planet Studio has no active terrain profile.")
		return
	var shape_slot: Resource = terrain.call("find_shader_slot",
		NATIVE.PRODUCTION_SHAPE_SLOT_ID) as Resource
	if shape_slot == null:
		_fail("Phase 41 Planet Studio has no resident Base Terrain Shape slot.")
		return
	...
	print("LATITUDE_MASK_PHASE41_UI_OK: generic displacement exposes and edits Latitude Band; resident Base Terrain keeps unsupported spatial masking hidden")
	editor.queue_free()
	await _frames(2)
	get_tree().quit(0)
