...
func reanalyse(center_dir: Vector3, radius_m: float) -> Dictionary:
	...
	if fields == null or grid == null:
		return {"ok": false, "reason": "Planet fields have incompatible runtime types."}
	...
	area.resize(n)
	for c in n:
		var cell_area: float = grid.cell_size[c] * grid.cell_size[c]
		area[c] = cell_area
	...
	for i in range(n - 1, -1, -1):
		...
		if order_value > max1[receiver]:
			max1[receiver] = order_value
			count1[receiver] = 1
		elif order_value == max1[receiver]:
			count1[receiver] = mini(255, int(count1[receiver]) + 1)
		elif count1[receiver] == 0:
			count1[receiver] = 1
	...
	for c in n:
		...
		var height: float = fields.elev[c]
		var filled: float = router.filled[c]
		...
		var base := c * 8
		var relief := 0.0
		var near_river: float = width[c]
		var near_level := height
		for k in 8:
			var neighbor: int = grid.nbr[base + k]
			relief = maxf(relief, absf(height - fields.elev[neighbor]))
			if width[neighbor] > near_river:
				near_river = width[neighbor]
				near_level = fields.elev[neighbor]
		var slope := relief / maxf(grid.cell_size[c], 0.001)
		var flat := clampf(1.0 - slope * 34.0, 0.0, 1.0)
		var above := height - near_level
		var floodplain := flat * NoiseKit.smoothstepf(0.0, 45.0, near_river) * clampf(1.0 - above / 55.0, 0.0, 1.0)
