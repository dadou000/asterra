...
const NaturalFacialHairGenerator = preload("res://scripts/character/front_shell_beard_groom.gd")
...
func _setup_ui() -> void:
	...
	var note := Label.new()
	note.text = "30× is the tuned default. LOD0 uses the full value; distant LODs automatically reduce follicle population. Beard roots are sampled directly on the visible face/jaw shell and facial morphs are transferred per follicle root."
