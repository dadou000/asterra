...
const NaturalFacialHairGenerator = preload("res://scripts/character/face_attached_beard_groom.gd")
...
func _setup_ui() -> void:
	...
	var note := Label.new()
	note.text = "30× is the tuned default. LOD0 uses the full value; distant LODs automatically reduce follicle population. Beard roots are sampled directly on source skin triangles. A compact morph/bone attachment map rejects throat/neck skin per follicle and importance-samples the actual facial surface."
