...
# grid metadata and +1 cell indexing
rep(p,'[[vk::binding(1,0)]] RWStructuredBuffer<uint4> g_grid;\nstruct Push', '[[vk::binding(1,0)]] RWStructuredBuffer<uint4> g_grid;\nstruct Push')
...
# pixel bindings + grid sampler helper
...
# modify ParticleIncident return inputs by applying grid attenuation around its call site
...
# pipeline/buffer counts, grid size, bind
rep(p,'.pushConstantDwords = 32U,\n        .shaderResourceBuffers = 9U,', '.pushConstantDwords = 32U,\n        .shaderResourceBuffers = 10U,')
rep(p,'.vertexAttributes={},.vertexStrideBytes=0U,.pushConstantDwords=32U,.shaderResourceBuffers=9U,.sampledTextures=1U,', '.vertexAttributes={},.vertexStrideBytes=0U,.pushConstantDwords=32U,.shaderResourceBuffers=10U,.sampledTextures=1U,')
rep(p,'.pixelShader={.data=dropletPixel.bytecode.data(),.size=dropletPixel.bytecode.size()},.vertexAttributes={},.vertexStrideBytes=0U,.pushConstantDwords=32U,.shaderResourceBuffers=9U,.sampledTextures=1U,', '.pixelShader={.data=dropletPixel.bytecode.data(),.size=dropletPixel.bytecode.size()},.vertexAttributes={},.vertexStrideBytes=0U,.pushConstantDwords=32U,.shaderResourceBuffers=10U,.sampledTextures=1U,')
...
# bind grid and depth at new slot for all three draws. grid buffer binding is graphics buffer 9.
rep(p,'    commands.SetGraphicsBuffer(8U,*oit.localLights);\n    commands.SetGraphicsTexture(0U,depth);', '    commands.SetGraphicsBuffer(8U,*oit.localLights);\n    commands.SetGraphicsBuffer(9U,*particleLightGrid_);\n    commands.SetGraphicsTexture(0U,depth);')
# there are subsequent state.BindForGraphics + SetGraphicsTexture calls; insert grid after each bind if missing
