...
def once(path: str, old: str, new: str) -> None:
    ...
    if count != 1:
        raise SystemExit(f"{path}: expected one seam, found {count}: {old[:100]!r}")
    p.write_text(text.replace(old, new, 1), encoding="utf-8")


# -----------------------------------------------------------------------------
# World-model schema: persistent M38 output + per-particle behavior.
# -----------------------------------------------------------------------------
...
once(path, '''    batch.particles.reserve(particleBudget.admitted);
...
''', '''    const f32 particleLifetime = std::isfinite(entry.settings.particleLifetimeSeconds) ? std::max(entry.settings.particleLifetimeSeconds, 0.001F) : 2.0F;
    ...
    const u32 particleCandidateLimit =
''')
once(path, '''            .authority = sample.authority,
...
once(path, '''struct VolumeParticleGpuStateRecord
{
    math::Float3 positionMeters{};
    f32 authority{0.0F};
    math::Float3 velocityMetersPerSecond{};
...
static_assert(sizeof(VolumeParticleGpuStateRecord) == 96U);
''')

path = "engine/volume_render/src/VolumeParticleGpuState.cpp"
once(path, '''struct Spawn
{
...
# -----------------------------------------------------------------------------
# Persistent renderer: physical radius and authored optical response.
# -----------------------------------------------------------------------------
path = "engine/volume_render/src/VolumeParticleRenderer.cpp"
once(path, '''struct Particle
{
    float3 positionMeters;
    float authority;
    float3 velocityMetersPerSecond;
...
'''
if text.count(old_vs) != 2:
    raise SystemExit(f"{path}: expected two VSOutput blocks, found {text.count(old_vs)}")
text = text.replace(old_vs, new_vs)
old_invalid = '''        output.emission = 0.0;
        output.life = 0.0;
        return output;
'''
new_invalid = '''        output.emission = 0.0;
        output.life = 0.0;
        ...
        return output;
'''
if text.count(old_invalid) != 2:
    raise SystemExit(f"{path}: expected two invalid-return blocks, found {text.count(old_invalid)}")
text = text.replace(old_invalid, new_invalid)
p.write_text(text, encoding="utf-8")
once(path, '''    const float2 corner = corners[cornerIndex];
...
''', '''    const float2 corner = corners[cornerIndex];
    ...
    output.position = center;
''')
once(path, '''    output.life = saturate(
...
''', '''    output.life = saturate(
    ...
    return output;
''')
once(path, '''    const float3 densityColor = lerp(
...
# -----------------------------------------------------------------------------
# Studio UI: authored commands instead of transient singleton settings.
# -----------------------------------------------------------------------------
path = "engine/studio_ui/src/VolumeAuthoringM36.cpp"
p = Path(path)
text = p.read_text(encoding="utf-8")
start_marker = '''    auto& outputSettings =
        volume_representation::
            VolumeOutputs().Settings(
                *volumeId);
'''
end_marker = '''    if ((outputSettings.particlesEnabled ||
         outputSettings.surfaceDepositsEnabled) &&
'''
start = text.find(start_marker)
end = text.find(end_marker)
if start < 0 or end <= start:
    raise SystemExit("VolumeAuthoringM36.cpp: transient M38 settings block not found")
replacement = '''    bool particlesEnabled = volume->outputParticlesEnabled;
...
text = text.replace(end_marker, '''    if ((volume->outputParticlesEnabled ||
         volume->outputSurfaceDepositsEnabled) &&
''', 1)
p.write_text(text, encoding="utf-8")

print("M38 authored particle behavior patch applied.")
