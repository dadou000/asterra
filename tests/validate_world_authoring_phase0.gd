extends SceneTree
## Headless invariants for Planet Studio staging plus the Phase 1 authoring model.
...
func _init() -> void:
	...
	var biome_layer_id := String(biome_layer.get(&"layer_id"))
	...
	var output_id := String((graph_nodes[0] as Dictionary).get("id", ""))
	var input_id := String(graph.call("add_node", "GAME_INPUT", Vector2(80.0, 120.0), {"source": "terrain_height_m"}))
	...
	var asterra_id := String(body.get(&"body_id"))
	...
	var moon_id := String(created.get(&"body_id"))
