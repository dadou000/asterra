...
// Sparse-specific CFL scratch reset. The first 96 bytes preserve the established
// diagnostics ABI. Phase 4 appends one per-iteration maximum characteristic rate
// |u|+c divided by local cell size so mixed HydroLODs can share one safe timestep
// without changing the fixed-domain solver's control block.
...
void main() {
    ...
    control.iter_max_cfl_rate_bits = 0u;
    control.reserved2 = 0u;
}
