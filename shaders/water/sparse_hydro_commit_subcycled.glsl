...
// Canonicalize only physical HydroLOD slots which actually advanced on this
// fine-clock tick. Atlas A remains authoritative for every non-due level, so
// leaving its cells untouched removes the old A->B->A identity copy traffic.
//
// Scratch B is intentionally undefined/stale for non-due and unoccupied slots.
// Reserved-slot staging initializes both A and B before occupancy is published,
// and every consumer already gates state access through occupancy.
...
layout(set = 0, binding = 3, std430) readonly buffer CommitParams {
...
layout(set = 0, binding = 6, std430) readonly buffer StepParams {
    vec4 grid_dt;
    vec4 physics;
...
void main() {
    ...
    if (slot >= commit_params.config.z || occupied[slot] == 0) return;

    // A non-due slot never wrote B during this fine tick. Preserve canonical A
    // verbatim rather than reading/writing a full tile merely to copy identity.
    if (scheduled_dt(slot) <= 0.0) return;
