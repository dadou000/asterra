...
// Converts the current-state characteristic-speed reduction into one CFL-safe
// substep. It runs before every candidate update, so dam-break acceleration and
// other evolving velocities can tighten dt without a CPU synchronization point.
...
void main() {
    ...
    if (pc.iteration + 1u >= cap && control.remaining_dt > 1e-7) {
        // The fixed-domain prototype deliberately slows simulated time rather than
        // violating CFL. Phase 3 can carry this remainder into a tile scheduler.
