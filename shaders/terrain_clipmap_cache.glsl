...
// Incremental terrain synthesis for the toroidal geometry-clipmap cache.
// Each invocation generates one exact lattice sample for one LOD. Rendering then
// reuses these samples until that world cell leaves the 512x512 cache window.
...
layout(push_constant, std430) uniform Params {
    vec4 anchor_dir_radius;      // xyz anchor unit direction, w planet radius
    vec4 anchor_right_base;     // xyz anchor tangent right, w L0 spacing
    vec4 anchor_up_macro;       // xyz anchor tangent up, w macro face resolution
    vec4 context_detail;        // x context res, y seed, z detail strength
    ivec4 rect;                 // xy logical cell origin, zw dispatch size
    ivec4 meta;                 // x level, y cache res, z anchor generation
} pc;
...
vec3 gm_rand3(ivec3 p, uint seed) {
    ...
    h1 ^= h1 >> 16u;
    h2 ^= h2 >> 15u;
    return vec3(float(h0 & 0x00ffffffu),
...
vec3 surface_face_uv(vec3 dir) {
    ...
    if (ad.x >= ad.y && ad.x >= ad.z) {
        if (d.x >= 0.0) {
            ...
            face = 1; axis = vec3(-1,0,0); right = vec3(0,0,1);
        }
    } else if (ad.y >= ad.z) {
        if (d.y >= 0.0) {
        ...
        if (d.z >= 0.0) {
            face = 4; axis = vec3(0,0,1); right = vec3(1,0,0);
        } else {
            face = 5; axis = vec3(0,0,-1); right = vec3(-1,0,0);
        }
    }
...
vec4 landform_weights(float macro_h, vec4 soil, vec4 surface,
    ...
    float arid = clamp((1.0 - surface.g)
        * (1.0 - clamp(ctx_precip_mm(climate) / 900.0, 0.0, 1.0))
...
vec3 flow_world(vec3 dir, vec2 flow_xy) {
    ...
    return normalize(east * flow_xy.x + north * flow_xy.y);
}
...
float geomorph(vec3 dir, float spacing_m, float macro_h) {
    ...
    if (w6k > 0.001 && mountain > 0.08) {
        ...
        h += (mix(ridge, cells, 0.58) * 2.0 - 1.0)
            * 210.0 * mountain * hardness * w6k;
    }
    ...
    if (w420 > 0.001) {
        ...
        if (dot(flow, flow) > 0.1) {
            ...
            sample_m = along * 0.42 + (sample_m - along) * 1.45;
        }
    ...
    if (w120 > 0.001) {
        h += gm_fbm(world_m / 120.0, seed + 89u)
            * 4.5 * (1.0 - glacial * 0.6) * w120;
...
vec3 direction_for_anchor_offset(vec2 offset_m) {
    ...
    if (arc <= TANGENT_APPROX_MAX_ARC_M) {
        return normalize(center + right * (offset_m.x / radius)
            + up * (offset_m.y / radius));
    }
    float theta = min(arc / radius, MAX_PROJECTED_THETA);
...
uint cache_key(ivec2 cell, int level, int generation) {
    ...
    h *= 0x846ca68bu;
    h ^= h >> 16u;
    // Zero is reserved for an uninitialized cache texel. 2^24 is exactly
    // representable by float32, so the full 1..16777216 key range is safe.
...
void main() {
    ...
    float final_h = macro_h
        + geomorph(dir, spacing, macro_h) * coast_guard * pc.context_detail.z;

    // CACHE_RES is a power of two; signed two's-complement AND gives the exact
    // positive modulo for negative logical cells without integer division.
