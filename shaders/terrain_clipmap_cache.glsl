...
vec3 flow_world(vec3 dir,vec2 flow_xy){if(dot(flow_xy,flow_xy)<=1e-5)return vec3(0.0);vec3 basis_up=abs(dir.y)>0.999?vec3(0,0,1):vec3(0,1,0);vec3 east=normalize(cross(basis_up,dir));vec3 north=normalize(cross(dir,east));return normalize(east*flow_xy.x+north*flow_xy.y);}
float geomorph(vec3 dir,float spacing_m,float macro_h){float radius=pc.anchor_dir_radius.w;uint seed=uint(max(pc.context_detail.y,1.0));vec3 world_m=dir*radius;vec4 soil=ctx_soil(dir),surface=ctx_surface(dir),geology=ctx_geology(dir),structure=ctx_structure(dir),climate=ctx_climate(dir),hydro=ctx_hydro(dir),weights=landform_weights(macro_h,soil,surface,geology,structure,climate,hydro);float mountain=weights.r,arid=weights.g,glacial=weights.b,depositional=weights.a,hardness=1.20-geology.r*0.48,h=0.0;float w16k=band_weight(16000.0,spacing_m);if(w16k>0.001){vec3 p=gm_domain_warp(world_m/16000.0,seed+11u,0.8);h+=gm_fbm(p,seed+13u)*mix(24.0,125.0,mountain)*w16k;}float w6k=band_weight(6000.0,spacing_m);if(w6k>0.001&&mountain>0.08){vec3 p=gm_domain_warp(world_m/6000.0,seed+31u,1.1);float cells=gm_cellular_ridge(p,seed+37u),ridge=gm_ridged(p*1.55,seed+41u);h+=(mix(ridge,cells,0.58)*2.0-1.0)*210.0*mountain*hardness*w6k;}float w1400=band_weight(1400.0,spacing_m);if(w1400>0.001){vec3 p=gm_domain_warp(world_m/1400.0,seed+53u,0.72);float ridge=gm_ridged(p*1.25,seed+59u),detail=gm_fbm(p*2.1,seed+61u);h+=((ridge*2.0-1.0)*72.0*mountain+detail*24.0)*w1400;}float w420=band_weight(420.0,spacing_m);if(w420>0.001){vec3 flow=flow_world(dir,ctx_flow(hydro)),sample_m=world_m;if(dot(flow,flow)>0.1){float along_q=dot(sample_m,flow);vec3 along=flow*along_q;sample_m=along*0.42+(sample_m-along)*1.45;}vec3 p=gm_domain_warp(sample_m/420.0,seed+71u,0.55);float channel=pow(gm_ridged(p,seed+73u),4.6);h-=channel*mix(2.0,34.0,hydro.b)*(1.0-depositional*0.78)*w420;if(depositional>0.006){float fan=pow(gm_ridged(p*0.48+vec3(9.0),seed+79u),2.2);h+=fan*depositional*mix(1.0,12.0,surface.a)*w420;}}float w120=band_weight(120.0,spacing_m);if(w120>0.001){h+=gm_fbm(world_m/120.0,seed+89u)*4.5*(1.0-glacial*0.6)*w120;float dune_weight=arid*soil.r;if(dune_weight>0.004){float dune=gm_ridged(gm_domain_warp(world_m/180.0,seed+97u,0.45),seed+101u);h+=(dune*2.0-1.0)*9.0*dune_weight*w120;}}float w24=band_weight(24.0,spacing_m);if(w24>0.001)h+=gm_value(world_m/24.0,seed+109u)*0.9*w24;if(glacial>0.01){float ice=gm_fbm(world_m/2600.0+vec3(2.0,-7.0,5.0),seed+127u);h=mix(h,h*0.62+ice*52.0,glacial*0.72);}return h;}
