...
public:
    explicit Impl(u32 workerCount)
    {
        ...
        for (u32 index = 0; index < workerCount; ++index)
        {
            workers_[index]->thread = std::thread(
                [this, index]
                {
                    WorkerLoop(index);
                });
    ...
    void Wait(const std::shared_ptr<detail::JobGroupState>& group)
    {
        if (!group)
        {
            return;
...
private:
    ...
    void Enqueue(JobItem item)
    {
        ...
        {
            std::scoped_lock lock(workers_[workerIndex]->mutex);
            workers_[workerIndex]->queue.push_back(std::move(item));
            outstandingJobs_.fetch_add(1, std::memory_order_release);
        }

        wakeCondition_.notify_one();
    }

    bool TryTakeOwn(const u32 workerIndex, JobItem& item)
    {
    ...
    bool TrySteal(const u32 victimIndex, JobItem& item)
    {
    ...
    void Execute(JobItem item)
    {
        ...
        if (item.group)
        {
            const u64 previous = item.group->remaining.fetch_sub(
                1,
...
{
    impl_->Submit(std::move(job));
}

void JobSystem::Submit(JobGroup& group, Job job)
{
    impl_->Submit(group.state_, std::move(job));
}
