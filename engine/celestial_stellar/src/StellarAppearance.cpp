...
// interpolated lattice), matching the GPU stellar surface shader.
[[nodiscard]] f64 Hash3(
    const math::Double3 p,
    const f64 scale,
    const u64 seed) noexcept
{
    const math::Double3 q = p * scale;
    ...
    const auto fade =
        [](const f64 t) { return t * t * (3.0 - 2.0 * t); };
    const f64 wx = fade(q.x - fx);
    const f64 wy = fade(q.y - fy);
    const f64 wz = fade(q.z - fz);
    const auto lerp =
        [](const f64 a, const f64 b, const f64 t) { return a + (b - a) * t; };
