...
int main()
{
...

    {
        ...
        const auto systemObject = commands.CreateObject(
            ...
            "Helion",
            worldObject);
        const auto planetObject = commands.CreateObject(
            orbit::world_model::kCelestialBodyType,
            "Asterra",
            systemObject);
        const auto moonObject = commands.CreateObject(
            orbit::world_model::kCelestialBodyType,
            "Luma",
            planetObject);
        ...
        commands.SetProperty(
            ...
            orbit::math::Double3{10'000.0, 20'000.0, 30'000.0});
        commands.SetProperty(
            planetObject,
        ...
        commands.SetProperty(
            moonObject,
            orbit::world_model::kBodyParentPositionMeters,
            orbit::math::Double3{400'000'000.0, 0.0, 0.0});
        ...
        const auto systemId = composition.SystemForObject(systemObject);
        const auto planetId = composition.BodyForObject(planetObject);
        const auto moonId = composition.BodyForObject(moonObject);
        ...
        const auto* system = composition.Bodies().FindSystem(*systemId);
        const auto* planet = composition.Bodies().FindBody(*planetId);
        const auto* moon = composition.Bodies().FindBody(*moonId);

        if (system == nullptr || planet == nullptr || moon == nullptr)
        {
            return 3;
        }

        const auto* ellipsoid =
            std::get_if<orbit::universe::EllipsoidShape>(&planet->shape);
        ...
        const auto stableMoonId = composition.BodyForObject(moonObject);

        if (!stableMoonId.has_value() || *stableMoonId != *moonId)
        {
            return 9;
        }
        ...
        const auto* rebuiltSphere = rebuiltMoon != nullptr
            ? std::get_if<orbit::universe::SphereShape>(&rebuiltMoon->shape)
            : nullptr;
