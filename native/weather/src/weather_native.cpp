...
void WeatherNative::step_global(float dt) {
	...
	for (int y = 0; y < GLOBAL_H; ++y) {
		...
		float mx = std::max(TAU_F * PLANET_RADIUS_M * coslat / float(GLOBAL_W), 12000.0f);
		float my = PI_F * PLANET_RADIUS_M / float(GLOBAL_H);
		...
		for (int x = 0; x < GLOBAL_W; x += 8) {
			...
			p = _mm256_fmadd_ps(_mm256_sub_ps(eqp, p), _mm256_set1_ps(dt / 12000.0f), p);
			// Tuned so a synoptic 1e-4 s^-1 convergence changes pressure by hundreds,
			// not millions, of pascals per model step.
			p = _mm256_fnmadd_ps(div, _mm256_set1_ps(3.0e3f * dt), p);
			__m256 p_neighbour = _mm256_mul_ps(
				_mm256_add_ps(_mm256_add_ps(pe, pw), _mm256_add_ps(pn, ps)),
				_mm256_set1_ps(0.25f));
			p = _mm256_fmadd_ps(_mm256_sub_ps(p_neighbour, p), _mm256_set1_ps(0.10f), p);
			...
			__m256 rain = clamp8(_mm256_add_ps(
				_mm256_mul_ps(_mm256_sub_ps(c, _mm256_set1_ps(0.025f)), _mm256_set1_ps(3.4f)),
				_mm256_mul_ps(trigger, _mm256_set1_ps(0.65f))), 0.0f, 1.0f);
...
void WeatherNative::global_state_at_dir(const Vector3 &d, float &cloud, float &cape, float &rain,
	...
	float lon = std::atan2(n.z, n.x); if (lon < 0.0f) lon += TAU_F;
	float lat = std::asin(std::clamp(float(n.y), -1.0f, 1.0f));
	float x = lon / TAU_F * GLOBAL_W - 0.5f;
	float y = (HALF_PI_F - lat) / PI_F * GLOBAL_H - 0.5f;
...
void WeatherNative::step_local(float dt) {
	...
	for (int y = 0; y < LOCAL_H; ++y) {
		for (int x = 0; x < LOCAL_W; x += 8) {
			...
			__m256 pe = _mm256_i32gather_ps(l.pressure.data(), ei, 4);
			__m256 pw = _mm256_i32gather_ps(l.pressure.data(), wi, 4);
			__m256 pn = _mm256_i32gather_ps(l.pressure.data(), ni, 4);
			__m256 ps = _mm256_i32gather_ps(l.pressure.data(), si, 4);
			__m256 dpdx = _mm256_mul_ps(_mm256_sub_ps(pe, pw), _mm256_set1_ps(0.5f / cell));
			__m256 dpdy = _mm256_mul_ps(_mm256_sub_ps(ps, pn), _mm256_set1_ps(0.5f / cell));
			// The 2.2 km nest cannot explicitly propagate an acoustic pressure wave
			// with a 20 s step. Use a balanced mesoscale response instead of the raw
			// air-density acceleration, which otherwise flips grid cells between clamps.
			u = _mm256_fnmadd_ps(dpdx, _mm256_set1_ps(dt / 8.0f), u);
			v = _mm256_fnmadd_ps(dpdy, _mm256_set1_ps(dt / 8.0f), v);
			...
			p = _mm256_fnmadd_ps(div, _mm256_set1_ps(2.0e3f * dt), p);
			__m256 p_neighbour = _mm256_mul_ps(
				_mm256_add_ps(_mm256_add_ps(pe, pw), _mm256_add_ps(pn, ps)),
				_mm256_set1_ps(0.25f));
			p = _mm256_fmadd_ps(_mm256_sub_ps(p_neighbour, p), _mm256_set1_ps(0.18f), p);
			...
			__m256 rain = clamp8(_mm256_add_ps(
				_mm256_mul_ps(_mm256_sub_ps(c, _mm256_set1_ps(0.060f)), _mm256_set1_ps(3.2f)),
				_mm256_mul_ps(trigger, _mm256_set1_ps(0.65f))), 0.0f, 1.0f);
			...
			p = _mm256_fmadd_ps(_mm256_sub_ps(_mm256_load_ps(gp), p), _mm256_set1_ps(0.18f), p);
			...
			_mm256_storeu_ps(&l.nu[base], clamp8(_mm256_mul_ps(u, _mm256_set1_ps(0.992f)), -95.0f, 95.0f));
			_mm256_storeu_ps(&l.nv[base], clamp8(_mm256_mul_ps(v, _mm256_set1_ps(0.992f)), -85.0f, 85.0f));
...
PackedFloat32Array WeatherNative::get_global_weather_rgba() const {
	...
	for (int i = 0; i < GLOBAL_W * GLOBAL_H; ++i) {
		float storm = std::clamp(std::max(g.cape[i] - 400.0f, 0.0f) / 2800.0f
			+ std::abs(g.vort[i]) * 60.0f + g.precip[i] * 0.50f, 0.0f, 1.0f);
		w[i * 4 + 0] = std::clamp(0.04f + g.cloud[i] * 1.10f + g.humidity[i] * 0.46f, 0.0f, 1.0f);
		w[i * 4 + 1] = storm;
...
PackedFloat32Array WeatherNative::get_local_weather_rgba() const {
	...
	for (int i = 0; i < LOCAL_W * LOCAL_H; ++i) {
		float storm = std::clamp(std::max(l.cape[i] - 450.0f, 0.0f) / 4200.0f
			+ std::abs(l.vort[i]) * 18.0f + l.precip[i] * 0.40f, 0.0f, 1.0f);
		w[i * 4 + 0] = std::clamp(0.04f + l.cloud[i] * 1.15f + l.humidity[i] * 0.44f, 0.0f, 1.0f);
		w[i * 4 + 1] = storm;
