...
void WeatherNative::horizontal_pass(Atmosphere &a, bool is_global, float dt) {
	// Keep the proven transport/momentum/saturation solver, then replace its old
	...
	// Sparse-cloud budget. The original core still performs resolved saturation,
	// which is desirable, but its old 68%-RH sub-grid seed is discarded here.
	// Outside an activated storm there is effectively no persistent cloud below
	// 90-93% total-water RH, and even near saturation only a small sub-grid
	// fraction survives. This makes clear air the default state instead of merely
	// lowering the renderer's opacity after condensate has accumulated.
	for (int layer = 0; layer < LAYERS; ++layer) {
		const int off = a.layer_offset(layer);
		const float sf = sigma_temperature_factor(layer);
		const float onset_rh = layer <= 1 ? 0.90f : 0.93f;
		const float full_rh = layer <= 1 ? 0.985f : 0.995f;
		for (int c = 0; c < a.cells; ++c) {
			const int i = off + c;
			float liquid = std::max(a.nliquid[i], 0.0f);
			float ice = std::max(a.nice[i], 0.0f);
			const float cloud_total = liquid + ice;
			if (cloud_total <= 1e-10f) continue;
			...
			float desired = resolved;
			if (total_rh > onset_rh) {
				const float subgrid_pool = std::max(
					total_water - saturation * onset_rh - resolved, 0.0f);
				const float partial = severe_smooth01(
					(total_rh - onset_rh) / std::max(full_rh - onset_rh, 0.001f));
				// Only 1.5-10% of the available pre-saturation pool survives as
				// persistent cloud. Resolved supersaturation remains fully retained.
				desired += subgrid_pool * (0.015f + 0.085f * partial);
			}
			...
			// A genuinely released storm can keep the condensate generated by the
			// resolved solver; fair-weather humidity cannot.
			desired = std::lerp(desired, cloud_total, storm_keep * 0.985f);
			...
			const float liquid_tau = std::lerp(layer <= 1 ? 600.0f : 900.0f,
				4800.0f, storm_keep);
			const float ice_tau = std::lerp(1800.0f, 14400.0f, storm_keep);
