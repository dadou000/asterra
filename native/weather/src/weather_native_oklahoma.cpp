...
void WeatherNative::horizontal_pass(Atmosphere &a, bool is_global, float dt) {
	// Keep the advection, pressure, momentum and resolved saturation physics from
	// the baseline core. The pass below only replaces its overly permissive
	// sub-grid cloud persistence and maintains a CIN-release memory.
	...
	// Replace the old 68%-RH persistent condensate branch with a scarce-cloud
	// closure. Resolved supersaturation is retained exactly; sub-grid cloud begins
	// around 84-88% total RH and only a small saturated fraction is allowed until
	// the column is close to saturation. Activated storms are exempt so they can
	// remain optically deep and violent.
	for (int layer = 0; layer < LAYERS; ++layer) {
		const int off = a.layer_offset(layer);
		const float sf = sigma_temperature_factor(layer);
		const float onset_rh = layer <= 1 ? 0.84f : 0.88f;
		const float full_rh = layer <= 1 ? 0.96f : 0.98f;
		for (int c = 0; c < a.cells; ++c) {
			const int i = off + c;
			float liquid = std::max(a.nliquid[i], 0.0f);
			float ice = std::max(a.nice[i], 0.0f);
			float cloud_total = liquid + ice;
			if (cloud_total <= 1e-9f) continue;

			const float temperature = a.ntheta[i] * sf;
			...
			const float total_water = a.nq[i] + cloud_total;
			const float total_rh = total_water / saturation;
			const float resolved = std::max(total_water - saturation, 0.0f);
			const float subgrid_pool = std::max(
				total_water - saturation * onset_rh - resolved, 0.0f);
			const float partial = severe_smooth01((total_rh - onset_rh) / (full_rh - onset_rh));
			float desired = resolved + subgrid_pool * (0.10f + 0.18f * partial);

			const float active = std::clamp(a.convective_activation[c], 0.0f, 1.0f);
			const float precip_active = severe_smooth01((a.nprecip[c] - 0.04f) / 0.25f);
			const float storm_keep = std::clamp(std::max(active, precip_active), 0.0f, 1.0f);
			desired = std::lerp(desired, cloud_total, storm_keep * 0.96f);
			desired = std::min(desired, cloud_total);

			const float phase_ice = cloud_total > 1e-8f ? ice / cloud_total : 0.0f;
			const float desired_ice = desired * phase_ice;
			const float desired_liquid = desired - desired_ice;
			const float liquid_tau = std::lerp(layer <= 1 ? 1500.0f : 2700.0f,
				4200.0f, storm_keep);
			const float ice_tau = std::lerp(7200.0f, 18000.0f, storm_keep);
			liquid = desired_liquid + (liquid - desired_liquid) * std::exp(-dt / liquid_tau);
			ice = desired_ice + (ice - desired_ice) * std::exp(-dt / ice_tau);
			const float new_total = std::max(liquid + ice, 0.0f);
			const float evaporated = std::max(cloud_total - new_total, 0.0f);
			if (evaporated > 0.0f) {
				a.nq[i] = std::clamp(a.nq[i] + evaporated, 0.00001f, 0.032f);
...
void WeatherNative::vertical_pass(Atmosphere &a, bool is_global, float dt) {
	...
	for (int c = 0; c < a.cells; ++c) {
		const float active = std::clamp(a.convective_activation[c], 0.0f, 1.0f);
		if (active < 0.015f) {
			// A capped column can retain large CAPE without advertising a broad
			// renderer-visible updraft field.
		...
		for (int interface_index = 0; interface_index < INTERFACES; ++interface_index) {
			...
			// Convective condensate is transported more directly than scalar heat,
			// and rapidly glaciates aloft. This produces a compact tower and spreading
			// anvil without increasing horizontal cloud coverage.
...
static PackedFloat32Array remap_sparse_severe_weather(PackedFloat32Array values,
	...
	for (int c = 0; c < a.cells; ++c) {
		...
		// Invert the legacy 4000x presentation closure, then rebuild coverage with
		// a much gentler 900x mapping. Storm strength affects vertical structure in
		// the renderer and contributes only a tiny amount to horizontal footprint.
		...
		const float cloud = std::clamp(
			1.0f - std::exp(-condensate_proxy * 900.0f) + storm * 0.035f,
			0.0f, 1.0f);
		w[o] = cloud;
		w[o + 1] = storm;
	}
