...
double max_relative_humidity(const VoronoiDryTransport &transport,
...
int main() {
	try {
		...
		// Basic saturation-pressure sanity around the triple-point/freezing range.
		...
		const auto warm_hydro = transport.diagnose_hydrostatic(warm);
		double warm_min_t = 1e300;
		for (double t : warm_hydro.temperature_k) warm_min_t = std::min(warm_min_t, t);
		require(warm_min_t > 288.0,
			"warm condensation failed to increase temperature");
		...
		// Subsaturated air with cloud liquid must evaporate and cool. Choose a small
		// condensate loading so every level can evaporate it completely.
		...
		const auto evap_hydro = transport.diagnose_hydrostatic(evap);
		double evap_max_t = -1e300;
		for (double t : evap_hydro.temperature_k) evap_max_t = std::max(evap_max_t, t);
		require(evap_max_t < 288.0,
			"cloud evaporation failed to cool the air");
		require(max_relative_humidity(transport, evap, 0) < 1.0,
